...
kable(head(table1[c(1:4,241:244),]), caption = "Buoy Data in Every 6 Minutes", booktabs = T)
...
kable(table2[1:10,], caption = "Median Buoy Data in Every 6 Hours", booktabs = T) %>%
  kable_styling(latex_option = "striped")
```

Since hurricane data are collected every 6 hours and buoy data are collected every 6 minutes in this case, and we want to combine the two data, we need to summarize buoy data by every 6 hours. After we full join the two data, our data are as follows in Table 3 and Table 4.
...
table3_2 <- table3 %>% select(3,9:13) %>% 
  ...
  rename(ATMP6h = "ATMP_mid_6h") %>%
  rename(WTMP6h = "WTMP_mid_6h")
kable(table3_1[1:10,], caption = "Joined hurricane col 1 to 8", booktabs = T)
kable(table3_2[1:10,], caption = "Joined hurricane col 9 to 13", booktabs = T)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, out.height = '80%', out.width= '55%'}
plot1()
plot2()
```
